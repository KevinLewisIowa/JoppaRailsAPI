class ClientInteractionsController < ApplicationController
  ...
  def seen_and_serviced_report
    attendance_records = [];
    ClientInteraction.joins('LEFT JOIN clients c ON client_interactions.client_id = c.id LEFT JOIN location_camps lc ON client_interactions.location_camp_id = lc.id LEFT JOIN routes r ON lc.route_id = r.id').where('client_interactions.serviced_date BETWEEN ? AND ?', params[:fromDate], Date.parse(params[:toDate]).next_day(1)).select('r.name, client_interactions.client_id, c.first_name, c.preferred_name, c.last_name, client_interactions.created_at, client_interactions.was_seen, client_interactions.serviced, client_interactions.at_homeless_resource_center, client_interactions.location_camp_id, lc.name AS camp_name, client_interactions.serviced_date').order('r.id, client_interactions.serviced_date').find_each do |attendance|
      attendance_records.push(attendance)
    end
    @seen_and_serviced_report = attendance_records  
  ...
  def clientAttendanceHistory
    interactions = [];
    ClientInteraction.joins('JOIN location_camps lc ON client_interactions.location_camp_id = lc.id JOIN routes r ON lc.route_id = r.id').where('client_interactions.client_id = ? AND client_interactions.serviced_date BETWEEN ? AND ?', params[:clientId], params[:fromDate], Date.parse(params[:toDate]).next_day(1)).select('client_interactions.id, client_interactions.created_at, client_interactions.was_seen, client_interactions.serviced, client_interactions.at_homeless_resource_center, client_interactions.location_camp_id, client_interactions.serviced_date, lc.name AS camp_name, r.name').order('client_interactions.serviced_date').find_each do |attendance|
      interactions.push(attendance)
    end
    @seen_and_serviced_report = interactions
