class AdminController < ApplicationController
  ...
  def getAdminRouteUndeliveredItems
    @routeUndeliveredItems = Client.joins('INNER JOIN requested_items AS ri ON clients.id = ri.client_id')
        ...
        .select("r.name, clients.first_name, clients.preferred_name, clients.last_name, ri.id, ri.client_id, ri.item_description, ri.date_requested, ri.fulfilled, ri.created_at").where("ri.has_received = ?", false).group("r.name, clients.first_name, clients.preferred_name, clients.last_name, clients.id, ri.item_description, ri.date_requested, ri.created_at, ri.fulfilled")
