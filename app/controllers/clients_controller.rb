class ClientsController < ApplicationController
  ...
  def getClientsByName
    ...
    if @clientName == 'ALLCLIENTS'
      @clientList = Client.joins('LEFT JOIN location_camps as lc on lc.id = current_camp_id LEFT JOIN routes r ON lc.route_id = r.id LEFT JOIN client_pets cp ON cp.client_id = clients.id').order('r.name', :status).select('clients.id, clients.phone, clients.birth_date, clients.first_name, clients.preferred_name, clients.last_name, clients.status, clients.last_interaction_date, clients.household_id, clients.is_veteran, clients.is_aftercare, clients.gender, clients.first_time_homeless, clients.date_became_homeless, clients.homeless_reason, clients.due_to_covid, clients.current_camp_id, clients.boot_size, clients.shoe_size, clients.dwelling, clients.admin_notes, clients.joppa_apartment_number, array_to_string(array_agg(pet_type || \',\' || quantity), \', \') as "Pets", r.name as route_name, lc.id as camp_id, lc.name as camp_name').group('clients.id, r.name, lc.id, lc.name')
    else
      @clientList = Client.joins('LEFT JOIN location_camps as lc on lc.id = current_camp_id LEFT JOIN routes r ON lc.route_id = r.id LEFT JOIN client_pets cp ON cp.client_id = clients.id').where('preferred_name ILIKE ? or first_name ILIKE ? or last_name ILIKE ? or concat(first_name, \' \', last_name) ILIKE ?', '%' + @clientName + '%', '%' + @clientName + '%', '%' + @clientName + '%', '%' + @clientName + '%').order('r.name', :status).select('clients.id, clients.phone, clients.birth_date, clients.first_name, clients.preferred_name, clients.last_name, clients.status, clients.last_interaction_date, clients.household_id, clients.is_veteran, clients.is_aftercare, clients.gender, clients.first_time_homeless, clients.date_became_homeless, clients.homeless_reason, clients.due_to_covid, clients.current_camp_id, clients.boot_size, clients.shoe_size, clients.dwelling, clients.admin_notes, clients.joppa_apartment_number, array_to_string(array_agg(pet_type || \',\' || quantity), \', \') as "Pets", r.name as route_name, lc.id as camp_id, lc.name as camp_name, (concat(first_name,concat(' ' last_name)))').group('clients.id, r.name, lc.id, lc.name')
    end
